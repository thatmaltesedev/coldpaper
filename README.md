...
<!-- DEMO GIF PLACEHOLDER ———————————————————————————————————————————————
  Record this exact sequence (~25s):
...
- **Backups survive damage.** The redundancy slider (default 25%) adds parity
  ...
  the parity budget — tears, stains, a missing page — and the file still
...
- **Encryption is optional and boring.** AES-256-GCM, key derived with
  ...
  the passphrase, lose the backup. That's the deal with real encryption.
...
- **Nothing leaves your device.** Verify it yourself: open DevTools → Network
  tab, then back up and restore a file — you'll see zero requests (after the
  initial page load; with the service worker installed, not even that).
...
— it's a PWA and works fully offline after the first visit.
...
**Print advice:** 100% scale ("actual size"), plain black & white. Laser
toner is waterproof; inkjet mostly isn't. Standard 80 g/m² paper is fine —
...
A 100 KB file at default settings is ~150 codes ≈ 13 pages (plus cover).
Soft warning above 500 KB, hard cap 5 MB — above that, paper stops being the
right medium (see FAQ).

## Security notes & threat model

Honesty section. Read it before trusting the tool with anything important.
...
- **PBKDF2, not Argon2.** WebCrypto (the only cryptography this app will use —
  ...
  the real defence**: six-plus random words (diceware), not a password you can
  remember by being clever.
- **Paper is only as safe as the drawer it lives in.** An unencrypted backup
  is readable by anyone with a phone — that's the *point* (future-you included),
...
- **The web app can be compromised like any web app.** If GitHub Pages or this
  ...
  feed it. Mitigations: the app is tiny and auditable, all processing is
...
- **Integrity beats secrecy failure modes.** Every chunk carries a CRC-32;
  the file's SHA-256 travels inside the (possibly encrypted) payload and is
  verified on restore. GCM authenticates the ciphertext. A wrong passphrase
  is detected as such; silent corruption has to defeat three layers.

## FAQ

**How big a file can I back up?**
Hard cap 5 MB, soft warning at 500 KB. At default density 5 MB ≈ 630 pages —
technically fine, practically silly. Paper is for the irreplaceable kilobytes.
...
**Why QR codes and not DataMatrix / colour codes / fancy high-density film?**
...
OCR of 4,000 characters of base64 is *far* less reliable than QR's built-in
error correction, position detection and binary mode. QR **is** the OCR-proof
encoding.
...
- Higher-capacity page layouts (2×3 large-code mode for bad printers)

## License

[MIT](LICENSE). The format specification (FORMAT.md) is dedicated to the
public domain to the extent legally possible — decoders everywhere, please.
