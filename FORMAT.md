...
- **Damage-tolerant.** The file survives the loss of whole codes (tears,
  ...
  erasure coding. Damage *within* a code is the QR symbology's own ECC's job.
- **Boring cryptography.** AES-256-GCM with a PBKDF2-SHA256 key — nothing
  custom, implementable from any standard crypto library.
...
Restore is the mirror image and is specified in [§8](#8-restore-algorithm).
...
slotsPerGroup k = ceil(kTotal / G)          (data slots per group, see §7.2)
totalChunks   = kTotal + m × G
```

**CRC-32** is the ubiquitous IEEE 802.3 / zlib CRC: reflected polynomial
`0xEDB88320`, initial value `0xFFFFFFFF`, final XOR `0xFFFFFFFF` — the same
function as zlib's `crc32()` and Python's `binascii.crc32`. It is computed
over the 18 header bytes immediately followed by the S data bytes (the four
...
If flag bit 1 is set, `payload` is the encryption envelope of §6 and `inner`
is its decrypted plaintext.
...
- Key: `PBKDF2-HMAC-SHA256(passphrase → UTF-8 bytes, salt, 600 000 iterations,
...
All arithmetic happens in GF(2⁸) with reducing polynomial
`x⁸+x⁴+x³+x²+1` (`0x11D`) and generator element `2` — the same field QR codes
use internally.
...
`2·2 = 0x04`, `0x80·0x02 = 0x1D` (reduction), `inv(0x03) = 0xF4`.
...
Within one group with `k` data slots `d₀ … d_{k−1}` (each `S` bytes) and `m`
...
parity_t[s]  = ⊕_j  mul( C[t][j], d_j[s] )
```

`C` is a Cauchy matrix (element `1/(xₜ − y_j)` with `xₜ = k+t`, `y_j = j`,
subtraction being XOR). Because `xₜ` and `y_j` are 2k+m ≤ 255+… pairwise
distinct field elements, **every square submatrix of `[ I_k ; C ]` is
invertible** — which is exactly the property that lets *any* k of the k+m
shards reconstruct the group. Note `k + m ≤ 255` always holds (§4 validation).
...
1. For each chosen parity row `t`, fold the known data slots into the parity:
   ...
   `rhs_t[s] = parity_t[s] ⊕ ⊕_{j known} mul(C[t][j], d_j[s])`

2. Build the `e×e` matrix `A[r][c] = C[t_r][ M_c ]` (parity rows restricted to
   the missing columns). `A` is a submatrix of a Cauchy matrix, hence
   invertible.
...
4. Each missing slot: `d_{M_i}[s] = ⊕_r mul( A⁻¹[i][r], rhs_{t_r}[s] )`.
...
1. **Frame-validate** each payload (§4). Drop invalid ones. Group survivors by
   `backupId`; if several backups are present, ask the user which to restore —
   never mix.
...
3. **Completeness check**: for every group `g`, count
...
4. **Reconstruct** every group (§7.5) to obtain all data slots; write each
   real slot `(g, s)` into padded-payload position `(s·G + g) · S`.
5. Truncate the padded payload to `payloadLength`.
6. If flag bit 1: parse `salt ‖ iv ‖ ciphertext`, derive the key (§6), decrypt
   AES-256-GCM. On tag failure, report *wrong passphrase* and allow retry
   without rescanning.
7. Parse the metadata block (§5.1) off the front of `inner`.
...
- Damage *inside* a code (small stains, printer artifacts) is absorbed by the
  QR symbology's own error correction (level M ≈ 15% of codewords) and, when
  that fails, the code simply counts as lost — the CRC keeps mis-decodes out.
...
- Decoders MUST refuse payloads with a higher version than they understand,
  ...
  versions; the reverse must hold too: new decoders MUST keep reading v1
  forever).
...
2. CRC-32 (zlib flavour) — standard library in most languages.
3. The 22-byte frame parser + validation list of §4.
4. Grouping by backupId; dedupe by chunkIndex.
5. GF(256) exp/log tables (15 lines; §7.1) and the Cauchy row formula.
6. Gauss-Jordan inversion of an ≤ m×m matrix over GF(256).
7. The slot mapping of §7.2 including virtual zero slots.
8. Payload truncation to `payloadLength`.
9. Optional: PBKDF2-SHA256 (600 000 iterations) + AES-256-GCM — both in every
   mainstream crypto library. Check against the §6.4 vector.
10. Raw-DEFLATE decompression (zlib `inflateRaw`, Python `zlib.decompress(d, -15)`).
11. SHA-256 for final verification.

Total: a few hundred lines in Python/Go/Rust. The §6.4 and §7.4 vectors plus
one self-made backup give you end-to-end confidence.
...
Photograph or scan every page, run the reader over each image, concatenate
nothing — just feed each result through the checklist above. Order never
matters; duplicates never hurt.
