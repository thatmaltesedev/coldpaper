...
- `test/pdf-roundtrip.test.ts` builds the real PDF, renders it with pdf.js,
  ...
  another, and restores byte-identically. If your change breaks this, it
  broke the product promise.
- `test/qr-degradation.test.ts` is the print-and-scan damage simulation
  (rotation, scale, noise, blur) against the bundled decoder.
- `test/roundtrip.test.ts` and `test/erasure.test.ts` are pipeline property
  tests across presets, passphrases and loss patterns.
- `test/crypto.test.ts` and `test/rs-vector.test.ts` hold **frozen vectors
  that lock the code to FORMAT.md.** If one fails, the spec or the code
  moved; figure out which one is wrong before touching the vector.
...
- Zero network requests at runtime. No CDNs, no analytics, no telemetry.
  The empty-Network-tab claim is a feature.
